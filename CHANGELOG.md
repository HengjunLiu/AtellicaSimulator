...
#### 1. **手工操作模式**
- ✅ **LOAD/UNLOAD请求的手工处理**
  - 收到LOAD/UNLOAD请求后，显示UI提示框
  - LOAD请求：提示"请从LAS的IP{x}卸载标本"
  - UNLOAD请求：提示"请向LAS的IP{x}装载标本"
  - 用户点击"完成"按钮后，等待10秒再发送响应
  - 避免UI阻塞，使用后台线程处理延迟

- ✅ **样本ID管理**
  - UNLOAD请求：用户输入要卸载的样本ID
  - LOAD请求：使用LAS请求中的样本ID
  - 更新请求body中的样本ID信息
...
- ✅ **removed_samples列表**
  - 维护已移除标本ID列表
  - 标本手工弹出时记录到列表
  - Onboard Sample Info消息包含已移除标本信息

- ✅ **Onboard Sample Info通知**
  - 标本移除后自动发送通知
  - 包含在线样本数量和已移除样本信息
  - 非请求式发送，主动通知LAS
...
#### 3. **手工弹出功能增强**
- ✅ **失败状态响应**
  - 手工弹出失败时发送错误状态（0x06 Load Skipped）
  - 成功时发送正常状态（0x01 Success）
  - 区分成功和失败的状态响应

### 🐛 问题修复

#### 1. **死锁问题修复**
- ✅ **process_load_unload死锁**
  - 修复`with self.sample_lock`块内部调用`get_ready_to_load`导致的死锁
  - 直接在锁内部获取`ready_to_load`值，避免重复获取锁
  - 确保LOAD/UNLOAD响应能够及时发送

#### 2. **Carrier Occupancy判断修正**
- ✅ **协议合规性修正**
  - 根据协议文档修正carrier_occupancy判断逻辑
  - `0x02, 0x03`（有样本）→ LOAD请求
  - `0x01`（空载波）→ UNLOAD请求
  - 符合`Atellica Solution LAS Interface Guide`规范

#### 3. **UI阻塞问题修复**
- ✅ **后台线程处理**
  - 点击"完成"按钮后，使用后台线程等待10秒
  - 避免`time.sleep(10)`阻塞UI主线程
  - UI保持响应，用户可以继续操作
...
- **修改文件**:
  - `las/las.py` (+200行，手工模式支持)
  - `core/core.py` (+5行，死锁修复)
  - `ui/ui.py` (+1行，版本号更新)
  - `CHANGELOG.md` (+80行，v1.5.0版本记录)
...
#### 1. **配置系统增强**
- ✅ **超时和重试配置参数**
  - `handshake_timeout`: 30秒 - LAS连接后未发送handshake消息的超时时间
  - `initialization_wait_period`: 30秒 - 握手完成后初始化序列的等待时间
  - `initialization_complete_timeout`: 30秒 - 初始化序列完成的超时时间
  - `ack_timeout`: 1秒 - ACK/NACK响应超时时间
  - `max_ack_retries`: 5次 - 最大ACK重试次数
  - `keep_alive_interval`: 15秒 - 保活消息间隔
  - `keep_alive_inactivity_timeout`: 15秒 - 无活动超时时间
  - 以及各种业务消息的超时配置

#### 2. **LAS服务器核心增强**
- ✅ **消息跟踪机制**
  - 实现了消息跟踪功能，支持监控所有发送消息的超时情况
  - 在`_build_message`方法中添加了`track_message`参数
  - 维护`pending_messages`字典跟踪待处理消息

- ✅ **超时检测系统**
  - 实现了`_run_timeout_check`方法，定期检查消息超时
  - 支持按消息类型确定不同的超时时间
  - 详细的超时事件日志记录
...
- ✅ **连接重置处理**
  - 实现了`_handle_connection_reset`方法，处理连接重置逻辑
  - 完整的通信状态重置流程

#### 3. **超时场景处理**
- ✅ **握手超时处理**
  - LAS连接后30秒内未发送handshake消息
  - 主动断开连接，记录详细日志
  - 清理相关资源
...
- ✅ **长耗时机械动作超时处理**
  - 暂停当前机械动作
  - 保留载具锁定状态
  - LAS重新连接后通过传输状态请求恢复动作

#### 4. **协议合规性增强**
- ✅ **Wait Period实现**
  - `Wait Period Prior to Initiating Handshake`: 30秒
  - `Wait Period Prior to Entering Listening Mode`: 15秒
  - `Wait Period to complete initialization sequence after Handshake`: 30秒
...
- **修改文件**:
  - `config/config.py` (+20行，超时和重试配置)
  - `las/las.py` (+500行，完整的超时和重试机制实现)
  - `ui/ui.py` (+1行，版本号更新)
  - `CHANGELOG.md` (+100行，v1.4.0版本记录)
